...
expansion rate is checkable from a file rather than from trust. `SMELT_BENCH_MODEL`
...
runner refuses to run against uncommitted corpus changes.
...
confinement to `tier2.mjs`/`tier3.mjs`, and that `bench/` never enters the published
`files` list; `pnpm mutate` proves each of those can go red.
