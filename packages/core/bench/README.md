...
| `corpus/stack-trace.txt`      | stderr of `node -e "import('./dist/index.js').then(async (smelt) => { const store = new smelt.MemoryElisionStore(); store.put('const x = 1;\n'); store.retrieve('feedfacedeadbeef'); });"` after `pnpm build`, captured once in one working copy of this repo (a git worktree, one machine, one Node version) — the absolute paths, frame numbers and Node banner it embeds are specific to that capture, so a re-run, even here, will not byte-match; the committed bytes are canonical, and tier 1 measures those |
