 ...
 * must catch. Every guard file in a package's `test/guards/` exports
 * `MUTATIONS: GuardMutation[]` beside its assertions — the break lives with the check
 * that must notice it, so a guard and its proof-of-failure travel together and review
 ...
 *
 * The type lives here, once; each package's `test/guards/_mutations.ts` re-exports it,
 * so a guard still spells its import `./_mutations.ts` and the runner's textual anchor
 * — the `export const MUTATIONS: GuardMutation[] = [` declaration line — is unchanged.
 */
export interface GuardMutation {
  /** Stable, globally unique id — `pnpm mutate` reports it, and history tracks it. */
  readonly id: string;
  /**
   * The file to break: relative to the package's `src/` for `kind: 'src'` (the
  ...
  readonly why: string;
  /**
   * `'src'` breaks a copy of the package's `src` (read via `SMELT_GUARD_SRC`);
