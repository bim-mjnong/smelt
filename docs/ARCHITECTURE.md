...
| `scripts/mutate.mjs`                                    | **The meta-guard, as a thin runner.** Discovers the guard files — in every workspace package with a `test/guards/` directory — and applies each one's own `MUTATIONS` export — 79 mutations across 17 guards, each of which must go red. A survivor is reported as a hole in the guard, not the mutation; the counts in this row are verified, not typed — the runner fails when they drift from what the guard files hold.                                                          |
