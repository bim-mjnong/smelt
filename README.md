...
- **The honesty machinery** — eighteen guard suites (seventeen in the core, one guarding
  ...
  a mutation runner (`pnpm mutate`) that breaks the source on purpose — 82 mutations
  across 18 guards, each watched going red — and fails if a guard does not notice. Every
  a mutation runner (`pnpm mutate`) that breaks the source on purpose — 88 mutations
  across 19 guards, each watched going red — and fails if a guard does not notice. Every
